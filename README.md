...
+ [Почему не типовое решение или БСП](#step0); 🤷‍♂️
...
+ [Вклад пользователей YouTube](#step6); 🤝
...
# Вклад пользователей YouTube:

##### <a name="step5"></a> Вклад пользователей YouTube [(начало)](#pageup); 

🤝 @ivanviktorovich6171 - заметил, что нет проверки на отсутствие обработки!
Если использовать управление 🔐 доступом, то при null лучше прерывать 📛выполнение всего модуля (возврат) и сообщить 💬 что пошло не так. Для устранения этой проблемы будет достаточно подключить 🔗 обработку или использовать другой код проверки (повторюсь, если применять управление доступом, то главное — не допустить открытия формы при отсутствии обработки).

🤝 @MotoCrankshaft - предложил использовать проверку на отсутствие выборки (пустая)
